...
public class TestProgram{
	 ...
	 */
	  ...
	  */
	  ...
	  /**
	   * A basic test for duplicate
	   */
	  private static void testDuplicate(){
		  BDDModel model= new BDDModel();
			Program myProgram = new Program(4,0,100,0,0,0, model, true);
			VarBool te = new VarBool("te", "", model);	
			Process process = new Process("Te","P1", 1, 3, 3, 1, model, true);
			process.addVarBool(te);
			ConsBoolExp tt = new ConsBoolExp(true);
			NegBoolExp neg_te = new NegBoolExp(te);
			VarAssign assign = new VarAssign(te,neg_te);
			...
			//
			VarBool cafe = new VarBool("cafe", "", model);	
			Process processCafe = new Process("Cafe","P2", 1, 3, 3, 1, model, true);
			processCafe.addVarBool(cafe);
			NegBoolExp neg_cafe = new NegBoolExp(cafe);
			VarAssign assigncafe = new VarAssign(cafe,neg_cafe);
			
			Branch branchcafe = new Branch("", cafe, assigncafe, false, processCafe);
			
			processCafe.addBranch(branchcafe);
			
			
			Process dup = process.duplicate("P2");
			
			myProgram.addProcess(process);
			myProgram.addProcess(dup);
