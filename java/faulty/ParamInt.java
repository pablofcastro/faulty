...
public class ParamInt implements IntExp, Var{
	String name; // the name of the parameter
	VarInt reference; // the reference to the actual parameter
	
	/**
	 * @param name	the name of the param
	 * @param reference	the reference bound to the parameter
	 */
	public ParamInt(String name, VarInt reference){
		this.name = name;
    ...
    **/
    public BDD updatePrime(Expression exp){
       ...
       return reference.updatePrime(iexp);
     ...
     */
    public ParamInt duplicate(String instName, HashMap<VarBool, VarBool>  boolMap, HashMap<VarInt, VarInt> intMap, Process owner){    	
    	//return reference.duplicate(instName, boolMap, intMap, owner);
    	return new ParamInt(this.name, this.reference);
    }
     ...
     * @return
     */
    public ParamInt duplicate(String instName, HashMap<VarBool, VarBool>  boolMap, HashMap<VarInt, VarInt> intMap, HashMap<ParamBool, ParamBool> boolPars, HashMap<ParamInt, ParamInt> intPars, Process owner){    	
    	
        if (intPars.get(this) != null){
    		return intPars.get(this); // is the variable is in boolMap then we have a clone
    	}
    	else{
    		 return new ParamInt(this.name, this.reference);
    	}
     ...
     * @return
     */
    public ParamInt duplicate(String instName, HashMap<Var, Var>  dups, Process owner){    	
        if (dups.get(this) != null){
    		return (ParamInt) dups.get(this); // is the variable is in boolMap then we have a clone
    	}
    	else{
    		 return new ParamInt(this.name, this.reference);
    	}
